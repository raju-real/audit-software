    ...
    <div class="row">
        <div class="col-12">
            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                <h4 class="mb-sm-0 font-size-18">Steps of :- {{ $audit->audit_no }} {{ $audit->title ?? 'Audit Steps' }}</h4>
            </div>
        </div>
    </div>
                                            ...
                                            @if (isStepActive($audit->audit_steps, $index))
                                                ...
                                                @if ($step->status === 'draft' || $step->status === 'returned')
                                                    <a data-bs-toggle="tooltip" data-bs-placement="top" title="Start Audit"
                                                        href="{{ route('admin.auditor-step-questions', encrypt_decrypt($step->id,'encrypt')) }}" class="btn btn-sm btn-success"><i
